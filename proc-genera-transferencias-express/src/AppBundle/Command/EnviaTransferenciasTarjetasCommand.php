...
class EnviaTransferenciasTarjetasCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('envia:transferencia:express:tarjetas')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $transferencias = $em->getRepository("AppBundle:Transferencias")->findBy(array('emailEnviado' => null, 'estado' => 1));

        switch($transferencias[0]->getIdtransferenciaTipo()){
            case 1:
                $tipoMail=47;
                // debito
            break;
        ...
        $template = $em->getRepository("AppBundle:EmailsPymes")->find($tipoMail);
        ...
        foreach ($transferencias as $transferencia) {
            ...
            $detalles = $em->getRepository("AppBundle:TransferenciasDet")->findBy(array('idtransferencia' => $transferencia->getIdtransferencia()));

            if (!isset($arrClientesMails[$transferencia->getIdcliente()])) {
                $arrClientesMails[$transferencia->getIdcliente()] = $em->getRepository("AppBundle:Clientes")->find($transferencia->getIdcliente());
            }
            ...
            foreach ($detalles as $detalle) {
                ...
                if (!isset($zonas[$detalle->getIdzona()])) {
                    $zonas[$detalle->getIdzona()] = $em->getRepository("AppBundle:TransferenciasZonas")->find($detalle->getIdzona());
                }
                
                if (!isset($categorias[$detalle->getIdcategoria()])) {
                    $categorias[$detalle->getIdcategoria()] = $em->getRepository("AppBundle:TransferenciasCateg")->find($detalle->getIdcategoria());
                }
            ...
            $table .= "<tr>
                            ...
                            <b><span style=\"font-size:9.0pt\">Total a transferir</span></b><span style=\"font-size:9.0pt\"></span>
                            </div></td>
                            <td style=\"background:#C8C5AA;padding:.75pt .75pt .75pt .75pt\"><div class=\"MsoNormal\" align=\"center\" style=\"text-align:center\"><b>
                            <span style=\"font-size:9.0pt\">$ " . number_format($transferencia->getMontototal(), 2, ",", ".") . "</span></b><span style=\"font-size:9.0pt\">
                            </span></div></td>
                            </tr>
                        </tbody></table>";
